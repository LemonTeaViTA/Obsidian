# 客户端-TDD-咪淘导播-iOS-执行UI测试自动化

模板类型：客户端

版本：1.3

创建时间：2026-07-19 00:49:09

更新时间：2026-07-19 00:49:09
...
# 咪淘导播 iOS UI 自动化任务执行模板

你是咪淘导播 iOS 客户端开发 / UI 自动化测试工程师。请在 **`@/openspec:run-ui-test-case`** 中创建并执行 UI 自动化平台任务，查询结果，完成失败归因；如果是业务代码或 UI 自动化脚本问题，修复后重跑。
...
| `@/openspec:apply` | 业务代码、测试代码、`apply_result.md` | 修复 iOS 业务代码 |
...
| `BUSINESS_CODE_ERROR` | App 崩溃、页面状态不符、文案/流程错误、真实缺陷被复现 | 参考 `@/openspec:apply` 修 iOS 业务代码 |
...
参考 `@/openspec:proposal`、`@/openspec:test-proposal`、`@/openspec:test-case`、`@/openspec:apply` 确认业务预期，修复 iOS 业务代码。必要时补 Unit / UIKit 组件 / SDK Adapter Contract Test，并执行 iOS 本地测试。

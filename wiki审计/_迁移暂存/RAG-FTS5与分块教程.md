---
module: RAG
tags: [RAG, FTS5, SQLite, 倒排索引, BM25, 分块, trigram, 全文搜索]
difficulty: hard
last_reviewed: 2026-06-01
---

# FTS5 与分块：SQLite 轻量化 RAG 原理

> 本文讲清 SQLite FTS5 全文搜索 + 滑动窗口分块的通用原理与完整检索流程——从「FTS5 是什么」「倒排索引内部结构」「BM25 排序本质」到「用真实例子走完整流程 Step 1–6」「与按段落切的对比」。
>
> 这是 RAG 检索链路的通用知识；具体产品（如 OpenClaw Memory）怎么用 FTS5 + sqlite-vec 落地，见 [[Memory 实现对比]] §5.5 的源码级对比。

> [!note] 迁移说明
> 本文内容由 `wiki/Agent/主流 Coding Agent 实现对比/Memory 实现对比.md` §5.5 迁出（原为对比文档内嵌的通用 RAG 教程，按域内 SSoT 归位到 RAG 域）。目标位置：`wiki/RAG/`。
...
==小文件==（几十 KB）字符上限够用。==生产级 RAG==（百万级文档）才上语义分块。详见 [[分块策略]]。
...
- [[RAG基础与架构]] — RAG 整体架构与分块策略
- [[分块策略]] — 各类分块方式的取舍
- [[RAG检索策略]] — BM25 / 向量 / 混合检索

...
> [!tip] 速览——本文导航
> - **同步工具类** — CountDownLatch（等待多线程）、CyclicBarrier（多线程互等）、Semaphore（限流）、Exchanger
> - **并发容器** — ConcurrentHashMap（分段/CAS）、CopyOnWriteArrayList、BlockingQueue 体系


# 并发工具类

> [!info] 速览
> 本篇覆盖 JUC 同步工具类与并发容器两大块：
> - 同步工具类：CountDownLatch、CyclicBarrier、Semaphore、Exchanger
> - 并发容器：ConcurrentHashMap（JDK7 分段锁 → JDK8 CAS+synchronized 桶锁）、CopyOnWriteArrayList、BlockingQueue
> 关键对比：CountDownLatch（一个人等多个人，一次性）vs CyclicBarrier（多个人互相等，可重用）。

## 同步工具类

### CountDownLatch 了解吗？

> [!tip] CountDownLatch vs CyclicBarrier 最关键的区别
> CountDownLatch 是一次性的（计数器归零不能重置），CyclicBarrier 可以重复使用。面试口诀：CountDownLatch 是"一个人等多个人"（主线程等子线程），CyclicBarrier 是"多个人互相等"（所有线程到达屏障点后一起继续）。
...
#### 说一下 JDK 8 中 ConcurrentHashMap 的实现原理？
 ...
 JDK 8 中的 ConcurrentHashMap 取消了分段锁，采用 CAS + synchronized 来实现更细粒度的桶锁，并且使用[[HashMap核心原理#你对红黑树了解多少？|红黑树]]来优化链表以提高哈希冲突时的查询效率，性能比 JDK 7 有了很大的提升。

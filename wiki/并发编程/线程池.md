...
> [!tip] 速览——本文导航
> - **线程池基础** — 七大参数、拒绝策略、任务调度流程
> - **线程池管理** — 动态调参、监控指标、优雅关闭（shutdown vs shutdownNow）
> - **线程池类型** — Fixed/Cached/Scheduled/Single 各自适用场景
> - **线程池实战** — 核心线程数计算公式（CPU密集/IO密集）、常见坑
...
> [!info] 速览
> 本篇覆盖线程池的原理、管理与实战四大块：
> - 线程池基础：作用、工作流程、七大核心参数、四种拒绝策略、阻塞队列、execute/submit 区别
> - 线程池管理：shutdown/shutdownNow、线程数配置（CPU 密集 vs IO 密集）
> - 线程池类型：四种常见线程池及其本质（都是 ThreadPoolExecutor 的不同配置）、5 种状态、异常处理、动态调参
> - 线程池实战：手写线程池/连接池、断电恢复
> 重点死磕七参数与任务流转流程，阿里规约要求手动 new ThreadPoolExecutor 而非用 Executors。
...
> [!tip] 线程池参数设计经验
> CPU 密集型：corePoolSize = CPU 核心数 + 1；IO 密集型：corePoolSize = CPU 核心数 × 2。但这只是起点——实际要通过压测确定。阿里开发手册明确禁止使用 Executors 创建线程池，必须通过 ThreadPoolExecutor 手动指定参数。
...
首先我会根据任务类型设置核心线程数参数，比如 IO 密集型任务会设置为 CPU 核心数2 的经验值。其次我会结合线程池动态调整的能力，在流量波动时通过 setCorePoolSize 平滑扩容，或者直接使用 DynamicTp 实现线程池参数的自动化调整。

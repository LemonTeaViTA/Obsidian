...
> [!tip] 速览——本文导航
> - **CAS 与原子操作** — compareAndSwap 原理、ABA 问题、AtomicReference
> - **synchronized** — 偏向锁→轻量级锁→重量级锁升级流程、Monitor、锁消除
> - **ReentrantLock 与 AQS** — CLH 队列、tryAcquire、公平/非公平锁
> - **死锁** — 四条件、检测、预防（资源排序、tryLock 超时）
> - **锁的分类** — 乐观/悲观、共享/排他、可重入、自旋
...
> [!info] 速览
> 本篇覆盖 Java 锁体系的四大块：
> - CAS 与原子操作：乐观锁原理、ABA/自旋开销/多变量三大问题、原子类
> - synchronized：上锁对象、Monitor 实现、可见性/有序性/可重入、锁升级（无锁→偏向→轻量级→重量级）
> - ReentrantLock 与 AQS：对比 synchronized、AQS state+CLH 队列、公平/非公平锁
> - 死锁与锁分类：四个条件与排查、自旋锁、悲观/乐观锁
> 建议先掌握 CAS 与 synchronized 锁升级，这两块是面试重灾区。
...
#### 怎么保证 CAS 的原子性？
 在 x86 架构下，CAS 底层是通过带有 `lock` 前缀的汇编指令（如 `lock cmpxchg`）实现的。
只要数据已经被缓存在了 CPU 的 Cache Line 中，并且没有跨缓存行，CPU 就会使用优化后的**缓存锁定（Cache Locking）**机制。它配合底层的缓存一致性协议（如 MESI）**，在缓存内部完成修改，并阻止其他核心修改同一内存地址，从而保证原子性。

只有当数据跨缓存行，或者数据本身不可缓存时，CPU 才会退化使用极其消耗性能的**总线锁定**。
...
```
 ...
 关键在于：观察线程要**先拿到旧版本号**，等值经历 A→B→A 之后，再用旧版本号做 CAS——此时值虽然变回了 A，但 stamp 已经被推进，CAS 失败，从而识破 ABA。
```java
class ABAFix {
	private static AtomicStampedReference<String> ref = new AtomicStampedReference<>("100", 1);

	public static void main(String[] args) throws InterruptedException {
		// 观察线程：先记录旧版本号 1，再等修改线程完成 A→B→A
		Thread observer = new Thread(() -> {
			int oldStamp = ref.getStamp(); // 此刻版本号 = 1
			try {
				Thread.sleep(100); // 等修改线程把 100→200→100 走完
			} catch (InterruptedException e) {
				Thread.currentThread().interrupt();
			}
			// 值虽然又变回 "100"，但版本号已从 1 推进到 3，CAS 失败
			boolean success = ref.compareAndSet("100", "300", oldStamp, oldStamp + 1);
			System.out.println("CAS 结果：" + success); // false，stamp 拦住了 ABA
		});

		// 修改线程：100 → 200 → 100，每步都推进版本号
		Thread modifier = new Thread(() -> {
			ref.compareAndSet("100", "200", ref.getStamp(), ref.getStamp() + 1); // stamp 1→2
			ref.compareAndSet("200", "100", ref.getStamp(), ref.getStamp() + 1); // stamp 2→3
		});

		observer.start();
		Thread.sleep(10); // 确保观察线程先读到旧版本号 1
		modifier.start();
	}
}
```
 如果把上面的 AtomicStampedReference 换成普通的 AtomicReference（不带版本号），最后那次 CAS("100"→"300") 就会**成功**，因为它只比较值、看不到中间的 B，这正是 ABA 问题的体现。
...
volatile 保证可见性和禁止指令重排，但不保证原子性。详见 [[线程基础与ThreadLocal#volatile 关键字的作用？]]。
...
```java
 // --- 1. 加锁准备阶段 ---
  0: aload_0         // 将 lock 对象的引用压入操作数栈顶（假设 lock 存在第0个槽位）
  1: dup             // 复制栈顶的 lock 引用
  2: astore 4        // 将复制的 lock 引用备份到局部变量表第 4 个槽位（为后续解锁做准备）
  4: monitorenter    // 消耗栈顶的 lock 引用，获取锁！进入同步代码块

  // --- 2. 你的同步代码块内部逻辑 (x = 1; flag = true;) ---
  5: iconst_1        // 将整数 1 压入操作数栈
  6: istore_1        // 存储 1 到局部变量 x (槽位 1)
  7: iconst_1        // 再次将整数 1 压入操作数栈 (true 在底层也是 1)
  8: istore_2        // 存储 1 到局部变量 flag (槽位 2)

  // --- 3. 正常执行完毕，释放锁 ---
  9: aload 4         // 加载刚才备份在第 4 个槽位的 lock 对象引用
 11: monitorexit     // 正常释放锁，退出同步代码块
 12: goto 21         // 正常执行完毕，跳过下方的异常处理代码

  // --- 4. 异常兜底阶段 (隐式的 finally 块，防止死锁) ---
 15: astore_3        // 如果第 5~8 行发生异常，捕获该异常并存入槽位 3
 16: aload 4         // 加载第 4 个槽位的 lock 对象引用
 18: monitorexit     // 发生异常时，强制释放锁！
 19: aload_3         // 将刚才捕获的异常重新压入栈顶
 20: athrow          // 将异常抛出给上层调用者
...
> [!warning] 锁升级不可逆
> synchronized 的锁升级是单向的：无锁 → 偏向锁 → 轻量级锁 → 重量级锁。一旦升级为重量级锁就无法降级。面试中常被追问"偏向锁撤销的开销"——撤销需要等到全局安全点（Safepoint），会造成 STW，这就是 JDK 15 默认关闭偏向锁的原因。
...
**③、轻量级锁**，当多个线程在不同时段获取同一把锁，即不存在锁竞争的情况时，JVM 会采用轻量级锁来避免线程阻塞。
 ...
 当线程进入 synchronized 加锁的代码时，如果对象处于无锁/未偏向状态，也就是锁标志位为“01”、偏向标记位为“0”的状态。
...
> [!note] Mark Word 锁标志位对照
> - `01` + 偏向标记 `0`：无锁（未偏向）
> - `01` + 偏向标记 `1`：偏向锁
> - `00`：轻量级锁（Mark Word 存指向 Lock Record 的指针）
> - `10`：重量级锁（Mark Word 存指向 Monitor 的指针）
> - `11`：GC 标记
...
| :--------- | :------------------------------------------------------------ | :----------------------------------------------------------------- |
| **底层实现机制** | **JVM 层面**：依赖底层的 Monitor（监视器锁），基于 C++ 实现和操作系统的 Mutex Lock 映射。 | **API 层面（JDK）**：基于 Java 原生的 AQS (AbstractQueuedSynchronizer) 框架实现。 |
| **锁的释放方式** | **自动释放**：发生异常或执行完代码块时，JVM 会自动释放锁，**不会死锁**。                    | **手动释放**：必须显式调用 `unlock()`，且**建议放在 `finally` 块中**。                 |
| **锁的灵活性**  | **较局限**：只能修饰方法或代码块，锁的范围被词法作用域写死。                              | **极高**：只要持有锁引用，可以在跨方法、跨类的流程中加锁和解锁。                                 |
| **公平性选择**  | **非公平锁**：只能是非公平模式。                                            | **可配置**：默认非公平，可传参实现**公平锁**。                                        |
| **中断响应机制** | **不可中断**：等待获取锁时会一直阻塞，无法被中断。                                   | **可中断**：支持 `lockInterruptibly()`，等待时可响应中断。                         |
| **多路通知**   | **单路通知**：结合 `wait()`/`notify()`，所有线程在同一个等待队列。                 | **多路通知**：可绑定多个 `Condition`，实现**精准唤醒**特定线程。                         |
| **锁状态探测**  | **不支持**：无法判断是否成功获取锁。                                          | **支持**：提供 `tryLock()`、`isLocked()` 等 API。                          |
| **性能表现**   | 引入锁升级优化后，**低中等竞争下性能极佳**。                                      | **高并发、高竞争**环境下，排队机制表现更稳定。                                          |
...
### AQS (AbstractQueuedSynchronizer)了解多少？
...
```
 第二、同步队列由内部定义的 Node 类实现，每个 Node 包含了等待状态、前后节点、线程的引用等，是一个先进先出的双向链表。
```java
...
#### 死锁发生的四个条件了解吗？
 ...
 第一条件是 互斥条件 ：资源不能被多个线程共享，一次只能由一个线程使用。如果一个线程已经占用了一个资源，其他请求该资源的线程必须等待，直到资源被释放。
 第二个条件是 请求与保持：一个线程已经持有一个资源，并且在等待获取其他线程持有的资源。
 第三个条件是 不可剥夺 ：资源不能被强制从线程中夺走，必须等线程自己释放。

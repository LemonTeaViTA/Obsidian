...
**第四步**，注册自动配置类。Boot 3.x（Spring 6 + JDK 17）使用 `src/main/resources/META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports` 文件，每行一个全限定类名：
...
> [!note] 兼容旧版本
> Boot 2.7 之前用的是 `src/main/resources/META-INF/spring.factories`，以 `EnableAutoConfiguration` 为 key 注册：
> ```properties
> org.springframework.boot.autoconfigure.EnableAutoConfiguration=\
> com.example.mystarter.autoconfigure.MyServiceAutoConfiguration
> ```
> Boot 2.7 起 `spring.factories` 注册自动配置已被标记废弃，3.0 起彻底移除，新项目直接用 `AutoConfiguration.imports`。
...
自动配置类的注册清单是 Spring Boot 自动装配的核心：Boot 2.7 之前位于每个 starter 的 `META-INF/spring.factories`，3.0 起改为 `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports`。Spring Boot 在启动时会读取这些清单，加载对应的配置类。

> [!warning] Spring Boot 3.0 自动装配不再用 spring.factories
> Spring Boot 3.0（Spring 6）起，自动装配改用 `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports` 文件替代 `spring.factories`。如果你的项目用的是 Boot 3.x + JDK 17，面试时说 `spring.factories` 后要补一句"3.0 已迁移到新机制"。

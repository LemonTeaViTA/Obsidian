### 🌟说说你对Spring事务的理解？ 
 ...
 ----这部分可以不背，方便大家理解 end---- 
 Spring 事务的底层实现是通过 [[AOP 与动态代理|AOP]] 来完成的。当我们在方法上加 @Transactional 注解后，Spring 会为这个 Bean 创建代理对象，在方法执行前开启事务，方法正常返回时提交事务，如果方法抛出异常就回滚事务。 
 ...
 事务能在新线程中传播吗？ 
 事务传播机制是通过 [[线程基础与ThreadLocal|ThreadLocal]] 实现的，所以，如果调用的方法是在新线程中，事务传播就会失效。 
 @Transactional 
 ...
 我先说一下 JDK [[AOP 与动态代理|动态代理]]，它要求目标类必须实现一个或者多个接口。也就意味着代理只能拦截接口中声明的方法，而 protected 和 private 方法并不能在接口中声明，因此在 JDK 动态代理下，这些方法的事务注解是会被直接忽略的。 

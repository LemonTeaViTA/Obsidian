...
> [!tip] 生产环境推荐 everysec
> `always` 每条命令都 fsync，QPS 直接掉到几百，只适合金融级零容忍场景。`no` 性能最高但可能丢数十秒数据。**everysec 是最佳平衡点**——最多丢 1 秒数据，性能几乎和关闭 AOF 一样。
...
> [!tip] Redis 5.0+ 默认开启混合持久化
> `aof-use-rdb-preamble yes` 是 Redis 5.0 之后的默认配置。如果你用的是 Redis 5.0+，AOF 文件自动就是混合格式，不需要额外配置。面试时说"我推荐开启混合持久化"再补一句"5.0 之后已经是默认行为"，能展示你对版本差异的了解。

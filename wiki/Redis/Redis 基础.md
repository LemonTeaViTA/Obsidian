...
部署过单机版、Docker 容器版，以及哨兵（一主两从 + 三个 Sentinel）和 Redis Cluster（3 主 3 从）。具体的安装步骤、`redis.conf` 配置、主从复制原理、哨兵选举机制和 Cluster 哈希槽分片见 [[高可用与集群]]。
...
如果是秒杀接口，还可以用 Lua 脚本实现令牌桶等限流算法，限制每秒只能处理 N 个请求。限流算法与完整脚本见 [[Redis 实战场景#限流方案]]。
...
> 6.0 的多线程只用在网络 I/O 读写，命令执行依然是单线程，所以不需要加锁。面试别说"Redis 6.0 变成多线程了"——这会被扣分。详见下文「Redis 6.0 使用多线程是怎么回事」。Redis 会根据操作系统选择最优的 IO 多路复用技术，比如 Linux 下使用 epoll，macOS 下使用 kqueue 等。

...
> [!info] 速览
> 本篇覆盖 MySQL 的两条主线：① **SQL 基础**——表连接、三大范式、字段类型选型（varchar/char、DATETIME、DECIMAL）、SQL 执行顺序与常用函数；② **数据库架构**——分层架构（连接层/服务层/存储引擎层）、一条查询/更新语句的执行流程、段区页行的存储结构。
> 跳转：[[#SQL 基础]] · [[#数据库架构]]。索引细节见 [[索引机制详解]]，事务与隔离级别见 [[事务与MVCC]]，存储引擎与日志见 [[引擎与日志]]。
...
varchar 是可变长度的字符类型，单行最多可以容纳 65535 个**字节**（注意是字节、且为整行所有列共享的上限，不是字符数）。实际可存的字符数还要看字符集：utf8mb4 下一个字符最多 4 字节，单个 varchar 实际最多约 16383 字符。再扣掉 MySQL 用 1 到 2 个字节记录字符串长度的开销，所以单列字节上限实际可设到 65533。
...
另外，DATETIME 的默认值为 null；TIMESTAMP 的默认值为当前时间——CURRENT_TIMESTAMP，实际开发中更常用，因为可以自动更新。

存储空间上，自 MySQL 5.6.4 起 DATETIME 占 **5 个字节**（外加 0~3 字节存储小数秒精度，5.6.4 之前为 8 字节）；TIMESTAMP 占 **4 个字节**（同样外加 0~3 字节小数秒）。

> [!warning] TIMESTAMP 2038 年溢出问题
> TIMESTAMP 用 4 字节存秒数，最大只能到 **2038-01-19 03:14:07 UTC**——著名的"Y2K38 问题"。新项目建议用 DATETIME(6) 存到微秒，配合 Java 8 `LocalDateTime` 处理，绕开时区与溢出双重坑。
...
> [!tip] 现代优化器已经会自动改写 IN/EXISTS
> MySQL 5.6+ 的优化器对 IN 子查询做了"半连接"（semi-join）优化，把 IN/EXISTS 改写成同样的执行计划。所以在大多数情况下两者性能差不多，**写哪个看可读性即可**。但如果发现某条 SQL 慢，先用 EXPLAIN 看看 select_type 是否被改写为 SUBQUERY 还是 SEMIJOIN。
...
> [!warning] Java 端千万别用 double / float 接钱
> Java 后端要把 DECIMAL 字段映射为 `BigDecimal`，绝不能用 `double`/`float` 接收。`0.1 + 0.2 = 0.30000000000000004` 这个坑在金融场景会直接导致账目对不上。MyBatis 类型处理器默认就是把 DECIMAL → BigDecimal，别手贱改。

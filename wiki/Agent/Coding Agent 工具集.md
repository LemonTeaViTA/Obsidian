---
module: Agent
tags: [Agent, Coding Agent, Tool Use, Claude Code, Cursor]
difficulty: medium
...
> 本文聚焦 ==Coding Agent（Claude Code / Cursor / Aider / Continue）实际提供的工具集==——分类、底层实现、设计原则。
>
> 协议见 [[Function Calling]]；接入方式见 [[MCP 协议]]；产品对比见 [[AI 编程工具]]；代码检索底层见 [[Code RAG]]；==Skills 详解==见 [[Agent Skills 体系]]。
...
> [!warning] 联网工具的安全攻击面
> ==SSRF 防御==（协议白名单 / 内网屏蔽 / DNS rebinding / 重定向手动追 / 大小+频率限制）==是 Agent 上线前必过的安全审计项==——详见 [[Agent 安全模型#八SSRF-防御联网工具的-agent-时代特殊攻击面]]。
>
> ==关键认知==：==LLM 输出的 URL 可被 Prompt Injection 控制==，==攻击者用一段污染的网页内容就能诱导 Agent 偷云 IAM 凭证==。

...
| Skills | 复用过去的成功经验（流程手册 / 决策手册），渐进式披露 | Agent 的肌肉记忆 |
| Harness | 控制流引擎：真正执行 LLM 决定的操作，把结果反馈回去 | Agent 的执行器 |

> [!note] Harness 与 Action Executor 的关系
> 早期资料常把执行层叫 "Action Executor"——它只是 Harness 的一部分。==Harness 是更完整的概念==：除了执行动作，还管控制流模式（ReAct/Plan/Reflection）、工具集成、上下文拼接、子代理编排。详见 [[Harness Engineering]]。

...
==审计日志最容易踩的隐患==：工具调用参数里==经常带凭证==——`Authorization: Bearer xxx` / `api_key: sk-...` / `password: xxx` / `token: ghp_...`。==审计日志原样落盘 = 凭证泄露==。详细脱敏实现见 [[MCP 安全模型#三、审计日志的凭证脱敏]]。
...
> [!info] 该内容已迁移
> 「并行检索 / 语义缓存 / 两阶段检索（粗召回 + Reranker 精排）/ 动态 Top-K」属 RAG 检索链路的延迟优化，与「Agent 可观测性」主题关联弱，已迁出到 RAG 域（`wiki审计/_迁移暂存/RAG-延迟优化策略.md`，目标 `wiki/RAG/`）。其中与可观测/成本直接相关的「超时降级 / 模型降级」见 §五 Agent 成本控制。

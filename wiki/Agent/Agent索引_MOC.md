...
>
> 模型基础见 [[LLM索引_MOC]]；RAG 体系见 [[RAG基础与架构|RAG 体系]]。
...
### 3. 工具调用：Agent 怎么行动
 ...
 MCP 协议
==工具标准化接入协议==（2026 事实标准）——Host/Client/Server 三层架构、Streamable HTTP transport、Host 集成层（工具命名空间 / Schema 清洗 / Resources 虚拟化 / notifications）、Server 生命周期与异步启动。
👉 **[[MCP 协议]]**

 MCP Server 生态
[[MCP 协议]] 的==生态分册==——主流 MCP Server 分类清单 + CDP 浏览器自动化（isolated vs shared / 登录态访问）这一最复杂的 Server 类别的工程细节。
👉 **[[MCP Server 生态]]**

 MCP 安全模型
[[MCP 协议]] 的==安全分册==——MCP 把工具变成独立进程带来的安全特性、三道安全关卡、凭证脱敏与审计日志。与 [[Agent 安全模型]] 互补：本文聚焦 MCP 边界，后者讲 Agent 整体纵深防御。
👉 **[[MCP 安全模型]]**
...
>
> 主目录: ==[[主流 Coding Agent 实现对比/索引|主流 Coding Agent 实现对比]]==

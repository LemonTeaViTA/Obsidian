...
> 文档解析中"合并单元格展开"的处理逻辑就是基于这个机制，详见 [[文档解析综述#3.1 表格结构识别（TSR）]]。
...
每个 Sheet 是独立的 XML 文件（worksheets/sheet1.xml、sheet2.xml）。这里的 `r:` 是 Office Relationships 命名空间（`http://schemas.openxmlformats.org/officeDocument/2006/relationships`）的前缀：主文档里写 `r:id="rId1"`，与 `_rels/workbook.xml.rels` 里的 `<Relationship Id="rId1" Target="worksheets/sheet1.xml"/>` 通过同名的 `rId1` 匹配，从而映射到具体文件。命名区域可以让公式用 `=SUM(价格表)` 而不是冰冷的 `=SUM(销售数据!$A$1:$D$100)`。
...
备注页放在 `notesSlides/` 目录，与正文分离——这是 [[文档解析综述#4.4 PPT（pptx）：信息分散与隐藏]] 中"备注页分离"问题的内部原因。

...
> [!warning] 双亲委派被打破的三个经典场景
> ①SPI 机制（JDBC、SLF4J）：Bootstrap ClassLoader 加载的接口需要加载第三方实现，通过 Thread.currentThread().getContextClassLoader() 打破；②Tomcat：每个 Web 应用有自己的 ClassLoader，优先加载 WEB-INF/classes，实现应用间隔离；③OSGi/热部署：自定义 ClassLoader 实现模块化加载。

...
HotSpot 内置**两个 JIT 编译器**，定位不同——一句话：**C1 求快，C2 求好**。
...
> [!info] 什么是 profile？（理解分层编译的前提）
> **profile = 运行时收集的"代码行为统计数据"**——比如某方法被调了多少次、某个 `if` 分支 99% 走 true、某个 `List` 变量实际类型一直是 `ArrayList`。
>
> C2 的激进优化**依赖 profile**：拿到"99% 走 true"才敢按 true 优化，拿到"类型一直是 ArrayList"才敢去虚拟化内联。==这正是 JIT 比 C++ 强的地方==——C++ 静态编译时拿不到这些运行时数据。收集 profile 需要在代码里"插桩"（埋点统计），有少量开销，所以不是所有层级都全量收集（见下方分层编译）。
...
==分层编译 = 把"解释器 + C1 + C2"按 5 个层级组合起来的策略==。核心矛盾：C2 优化好但编译慢（要等攒够 profile），C1 快但优化弱。分层编译让代码**先用 C1 快速跑起来 + 顺便收集 profile，等够热了再上 C2 用这些 profile 做激进优化**——两全其美。

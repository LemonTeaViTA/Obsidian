...
当任一计数器超过阈值（C1 默认 1500，C2 默认 10000），就触发 JIT 编译。
...
JDK 9 之后，GraalVM 的 Graal 编译器（用 Java 写的）可以替代 C2（`-XX:+UseJVMCICompiler`），优化能力更强，尤其对 Scala/Kotlin 这种高阶函数密集的代码。
...
3. **AOT 编译**（JDK 9+，`jaotc`）：提前把字节码编译成本地镜像，绕过解释执行阶段
4. **CDS / AppCDS**：类数据共享，加速类加载（不解决 JIT 但改善启动）
5. **GraalVM Native Image**：彻底 AOT，无解释阶段，但失去 JIT 的运行时优化能力

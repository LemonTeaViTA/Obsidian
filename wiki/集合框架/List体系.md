...
> 面试别只说"ArrayList 查快改慢，LinkedList 改快查慢"。实际上由于 CPU 缓存行的局部性原理，ArrayList 在绝大多数场景（包括中间插入）都快于 LinkedList。LinkedList 的唯一优势场景是在已知节点引用的情况下进行 O(1) 删除，但 Java 的 LinkedList API 不暴露节点引用。

...
>
> 注：ToT 作为 **Agent 规划搜索架构**（树形展开 + 评估 + 剪枝 + 与 LATS 的关系）见 [[Plan 模式家族对比#三、Tree of Thoughts(ToT)]]。
...
> 传统 Prompt Engineering 关注"怎么引导模型思考"（CoT、Few-shot），现代 Context Engineering 关注"给模型组装什么信息"（系统指令、工具描述、检索结果、Memory）。详见 [[Agent 工程实践#Context Engineering 取代 Prompt Engineering]]。
...
- [[LLM 基础与训练]] — Prompt 的效果取决于模型能力
- [[Harness Engineering]] — Harness 是 Prompt 的进阶
- [[Agent 核心概念]] — Agent 依赖高质量 Prompt 驱动

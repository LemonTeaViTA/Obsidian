...
## 五、Embedding 与 Agentic RAG

==Agent 中的 RAG 内容已独立到 wiki/RAG/ 目录==——避免重复，本节仅做导航：

| 主题 | 详见 |
|------|------|
| Embedding 算法演进（四代） | [[RAG向量与Embedding#四、Embedding 演进]] |
| Embedding 模型选型 | [[RAG向量与Embedding#五、Embedding 模型选型]] |
| Agentic RAG 设计原则 | [[RAG高级技术#一、Agentic RAG（智能体 RAG）]] |
| Self-RAG / Reflection Tokens | [[RAG高级技术#三、自反思 RAG（Self-RAG）]] |
| RAG-Gym 过程监督 | [[RAG高级技术#RAG-Gym：过程监督与策略优化]] |
| 多轮对话检索去重 | [[RAG检索策略#四、多轮对话检索去重]] |
| RAG 评估方法论 | [[RAG评估]] |
| 幻觉检测与置信度 | [[幻觉与置信度]] |

==Agent 视角下的 RAG 应用==：把 RAG 作为 Agent 的一个工具暴露给 LLM（详见 [[RAG检索策略#1.1 Function Calling / Tool Use]]），让 Agent 自主决定何时检索。
...
### 工具权限控制：白名单 / 参数粒度 / Human-in-the-Loop

...
==六大组件解决"用什么工具"，控制流模式解决"怎么编排"==——也属于 Harness 层而非 LLM 层。三种主流控制流：

| 模式 | 核心思想 | 适用 |
|------|---------|------|
| ==ReAct== | Thought → Action → Observation 循环，LLM 走一步看一步 | 任务开放、需要探索的场景（编码 Agent、客服） |
| ==Plan-and-Execute== | LLM 先一次性规划 N 步，再逐步执行 | 任务步骤清晰、有依赖关系（报告生成） |
| ==Reflection== | 生成 → 反思 → 重生成 | 质量要求极高、允许多轮迭代（叠加在前两种上） |

==关键认知==：这三种==都是 Harness 层==实现的——LLM 提供"推理"和"输出 Action"的能力，Harness 实现循环控制、Observation 拼接、停止判断。==LLM 自己不会循环==——你不写循环代码，模型不会自己回头。

详细对比与选型见 [[Agent 核心概念#二、ReAct 框架与推理模式]]。

#### Function Calling vs ReAct vs Agentic RAG 三者关系

==经常被混淆==：

| 概念 | 层级 | 解决什么 |
|------|------|--------|
| ==Function Calling== | LLM 协议层 | 让 LLM 输出"调什么工具"的结构化 JSON |
| ==ReAct== | Harness 控制流 | 把 LLM 的 Action 和工具执行串成循环 |
| ==Agentic RAG / Coding Agent== | Application 层 | 用 ReAct + 多种工具实现具体业务能力 |

==上层依赖下层==。Claude Code、Cursor 这种 Coding Agent 都是==典型的 ReAct + 工具集成层==的产物——它们的核心创新不在 LLM，而在"工具集设计 + 控制流的精致度"。
...
> [!info] 本节汇总 Harness 的"会不会被淘汰"之争，以及四类工程化落地实践：UI 约束（DESIGN.md）、Prompt 分层架构、落地路径、开源参考实现。
...
### Prompt 分层架构：System Prompt 的工程化管理

==随着 Coding Agent 功能增长，system prompt 从几百字膨胀到几千字==——硬编码在代码里既难维护又难复用。==Prompt 分层架构==是把 system prompt 拆成多个职责清晰的层，按顺序组装。

#### 为什么需要分层

| 问题 | 硬编码 system prompt | 分层架构 |
|------|---------------------|---------|
| 多角色（ReAct / Planner / Reviewer） | 每个角色复制一份全量 prompt | ==共享 base 层，各自只写差异层== |
| 用户/项目级定制 | 改代码重新发布 | ==文件覆盖，不改代码== |
| 动态上下文（Skills / Memory） | 拼接逻辑散在代码里 | ==固定位置注入，顺序可控== |
| 审计 prompt 变更 | diff 代码 | ==diff Markdown 文件== |

#### 组装顺序：越靠后越接近 LLM 注意力焦点

==生产 Coding Agent 的标准组装顺序==（PaiCli / Claude Code 同构）：

```
base          → 身份定义 + 核心行为约束（最稳定，进 prompt cache）
personality   → 语气风格（简洁 / 详细 / 中文优先）
mode          → 当前推理模式（ReAct / Plan / Multi-Agent）
approval      → HITL 审批规则（危险操作怎么问用户）
project_context → CLAUDE.md / AGENTS.md 项目规则（项目级）
skills        → 当前启用的 Skill 索引（按需加载）
context_mgmt  → 上下文管理指令（历史截断 / token 预算）
handoff       → 当前任务交接信息（最动态，不进 cache）
```

==越靠后越动态==——`base` 几乎不变（进 prompt cache），`handoff` 每轮都变（不进 cache）。这与 [[长上下文工程#1-prompt-caching]] 的缓存点设计一致：==稳定内容放前面，变化内容放后面==。

#### 三层文件覆盖

==与 [[Agent Skills 体系#十二、目录位置]] 的三层加载完全同构==：

| 层级 | 路径 | 优先级 |
|------|------|--------|
| ==内置层== | jar 内 `resources/prompts/` | 最低（产品默认） |
| ==用户级== | `~/.paicli/prompts/` | 中 |
| ==项目级== | `.paicli/prompts/` | ==最高==（项目专属） |

==整文件替换，不是字段级合并==——与 Skill 覆盖原则一致，行为可预测。

#### PromptAssembler 实现

```python
class PromptAssembler:
    LAYERS = ["base", "personality", "mode", "approval",
              "project_context", "skills", "context_mgmt", "handoff"]

    def assemble(self, role: str, dynamic: dict) -> str:
        parts = []
        for layer in self.LAYERS:
            # 三层查找：project > user > builtin
            content = self._load_layer(role, layer)
            if content:
                # 动态变量替换（{{project_name}} 等）
                parts.append(content.format(**dynamic))
        return "\n\n".join(parts)

    def _load_layer(self, role: str, layer: str) -> str | None:
        for base in [self.project_dir, self.user_dir, self.builtin_dir]:
            path = base / "prompts" / role / f"{layer}.md"
            if path.exists():
...
| 原则 | 说明 |
|------|------|
| ==base 层必须包含 `## Language`== | 确保 LLM 知道用什么语言回复 |
| ==动态内容靠后注入== | `skills` / `context_mgmt` / `handoff` 在后面，不破坏前面层的 cache |
| ==覆盖是整文件替换== | 不做字段级 merge，避免"部分覆盖"导致语义矛盾 |
| ==prompt 变更要审计== | 用 `docs/prompt-analysis-template.md` 记录每次 prompt 改动的意图和影响 |

==面试时讲清==："==Prompt 分层架构是 Harness 工程化的核心==——把 system prompt 从硬编码字符串变成可版本化、可覆盖、可审计的文件系统。组装顺序决定 cache 效率：稳定层靠前进 cache，动态层靠后每轮变。这和 [[Agent Skills 体系]] 的渐进式披露、[[长上下文工程]] 的 prompt cache 设计是同一套思路的不同层面。"
...
- [[Prompt Engineering]] — Prompt 到 Harness 的演进
- [[Agent 工程实践]] — Harness 在 Agent 中的应用
- [[LLM 基础与训练]] — 模型能力是 Harness 的基础
